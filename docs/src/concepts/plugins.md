...
> **Naming note:** In the trait API, an Engine is called a `Plugin` for historical reasons. In the documentation we call this tier an **Engine** — it's the standard term used by SQLite, MySQL, and Tantivy for the same architectural layer. Both words refer to the same thing; the trait name is kept stable for API compatibility.

Axil's storage substrate is built from **Engines** — compile-time modules that implement the `Plugin` trait, own a companion file next to the core `.axil` database, and receive insert/update/delete lifecycle hooks from the master coordinator.
...
1. Implement the base `Plugin` trait plus one of the index traits (`VectorIndex`, `GraphIndex`, `SearchIndex`, `TimeSeriesIndex`) or define its own.

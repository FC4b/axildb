...
Load-time **ABI-version negotiation** (a precise error when a plugin's `axil:plugin@X.Y.Z` isn't one this host implements, instead of a raw link failure) and a **compiled-module cache** (`.axil/plugins/.cache/`, ~16× faster repeat invocations — a deserialize instead of a recompile) are in. Still ahead (Phase 22 polish): an ergonomic guest SDK (proc-macro) and a conformance/fuzz suite. None change the contract above.
